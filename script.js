...
async function menuju(){await swals.fire('', 'success');window.location = "https://api.whatsapp.com/send?phone=&text=" + pesanwhatsapp;Tombol.style="margin-top:15px;opacity:1;transform: scale(1);";}
...
function mulaiketik2() {if(ac<vketik2.length){kalimatb.innerHTML += vketik2.charAt(ac);ac++;setTimeout(mulaiketik2,45);} if(ac==vketik2.length){tombol();}}
  ...
  function stakhir(){tmbl.innerHTML="goodluck ulitt!";Tombol.style="opacity:1;transform: scale(1)";ftom=5;fungsi=0;}
  ...
  async function dibalas(){
   ...
   async function jawab(){
           var { value: jawaban } = await swals.fire({
               title: 'goodluck, iloveyouuuu sm! kiss kopo marami ha?', input: 'text', allowOutsideClick: false, showCancelButton: false,
           });
           if(jawaban && jawaban.length < 19){
               ...
               await swals.fire(message mo raw meh);
           }
       }
